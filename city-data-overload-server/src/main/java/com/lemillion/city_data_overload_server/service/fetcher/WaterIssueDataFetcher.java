...
/**
 * Fetcher for water-related issues from SerpApi
 */
@Service
@RequiredArgsConstructor
@Slf4j
public class WaterIssueDataFetcher implements BaseSerpApiFetcher {
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                String url = UriComponentsBuilder.fromHttpUrl("https://serpapi.com/search")
                    ...
                    .queryParam("tbm", "nws") // News search for water issues
                ...
                if (jsonResponse == null) {
                    log.warn("Received null response from SerpApi for water query: {}", query);
                    return null;
                }
                ...
                if (response != null && response.hasError()) {
                    ...
                    if (errorMsg != null && errorMsg.contains("Google hasn't returned any results")) {
                        log.debug("No water issue results found for query: {} - this is normal", query);
                        // Return an empty response instead of null
                        response = new SerpApiResponse();
                    } else {
                        log.warn("SerpApi error for water issue query '{}': {}", query, errorMsg);
                    }
                }
                ...
                log.error("Error fetching water issue data for query: {}", query, e);
                return null;
            }
        });
    }
    ...
    public List<CityEvent> convertToCityEvents(SerpApiResponse response) {
        ...
        if (response.getNewsResults() != null) {
            for (SerpApiResponse.NewsResult newsResult : response.getNewsResults()) {
                if (isWaterRelated(newsResult.getTitle(), newsResult.getSnippet())) {
        ...
        if (response.getOrganicResults() != null) {
            for (SerpApiResponse.OrganicResult organicResult : response.getOrganicResults()) {
                if (isWaterRelated(organicResult.getTitle(), organicResult.getSnippet())) {
    ...
    public List<String> getDefaultQueries() {
        return Arrays.asList(
            "water cut Bengaluru today",
            "water shortage Bangalore",
            "water supply issue Bengaluru",
            "BWSSB water cut Bangalore",
            "water tank empty Bengaluru",
            "water contamination Bangalore",
            "drinking water problem Bengaluru",
            "water pipeline burst Bangalore",
            "water logging Bengaluru",
            "water quality issue Bangalore",
            "water pressure low Bengaluru",
            "water supply disruption Bangalore"
        );
    }
    ...
    private CityEvent createEventFromNews(SerpApiResponse.NewsResult newsResult) {
        try {
            ...
            log.error("Error creating event from water news result: {}", newsResult.getTitle(), e);
            return null;
        }
    }
    ...
    private CityEvent createEventFromOrganic(SerpApiResponse.OrganicResult organicResult) {
        try {
            ...
            log.error("Error creating event from water organic result: {}", organicResult.getTitle(), e);
            return null;
        }
    }

    private boolean isWaterRelated(String title, String snippet) {
        String combined = (title + " " + snippet).toLowerCase();
        String[] waterKeywords = {
            "water", "bwssb", "supply", "cut", "shortage", "contamination", 
            "pipeline", "tank", "drinking", "quality", "pressure", "logging",
            "drainage", "sewage", "leak", "burst", "maintenance", "disruption"
        };
    ...
    private List<String> extractKeywords(String title, String content) {
        ...
        String[] waterKeywords = {
            "water", "supply", "cut", "shortage", "contamination", "pipeline",
            "tank", "quality", "pressure", "bwssb", "drainage", "sewage"
        };
