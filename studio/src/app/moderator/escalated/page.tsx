...
export default function ModeratorEscalatedAlertsPage() {
  ...
  React.useEffect(() => {
    const fetchEscalated = async () => {
      ...
      try {
        ...
        setEscalatedAlerts(moderatorAlerts.filter(a => a.isEscalated));
      } catch (error) {
    ...
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>High-priority content moderation cases requiring senior review.</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <Table>
              ...
              <TableBody>
                {escalatedAlerts.map((alert) => (
                  <TableRow key={alert.id}>
                    ...
                    <TableCell>{alert.snippet}</TableCell>
                    <TableCell>
