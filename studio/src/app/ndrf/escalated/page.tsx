...
export default function NdrfEscalatedAlertsPage() {
  ...
  React.useEffect(() => {
    const fetchEscalated = async () => {
      ...
      try {
        ...
        setEscalatedAlerts(ndrfAlerts.filter(a => a.isEscalated));
      } catch (error) {
    ...
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>High-priority alerts requiring immediate attention from the NDRF.</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <Table>
